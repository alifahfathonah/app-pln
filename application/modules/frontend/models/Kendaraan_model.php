...
class Kendaraan_model extends CI_Model
{
    ...
    function ubah_status_kendaraan($data)
    {
        $status = array('dokumen_status' => $data['status_dokumen']);
        $status_ = array('status_dokumen' => $data['status_dokumen']);
        $result = $this->db->where('id', $data['id'])->update('peminjaman_kendaraan', $status);
        $result = $this->db->where('id_peminjaman_kendaraan', $data['id'])->update('peminjaman_kendaraan_persetujuan', $status_);

        return $result;
    }

    function datadriver($id_peminjaman_kendaraan)
    {
        $this->db->select('k.nopol, us.nama as nama_driver, us.telp_wa as telp_driver');
        $this->db->from('peminjaman_kendaraan pk');
        $this->db->join('kendaraan k', 'k.id = pk.id_kendaraan', 'left');
        $this->db->join('users us', 'us.id = pk.id_driver', 'left');
        $this->db->where('pk.id', $id_peminjaman_kendaraan);
        $data['driver'] = $this->db->get()->row();

        $data['traveladmin'] = $this->db->select('telp_wa')->from('users')->where('id', '394')->get()->row();
        return $data;
    }
}
