<script>
    var dataTable = '';
    var baseUrl = '<?= base_url('frontend/kendaraan') ?>';
    ...
    $("#filter-kendaraan").keyup(function() {
        table_data();
    });
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.validasi === false) {
                    validation_select('#kendaraan', 'kendaraan', data);
                    validation('#tujuan', 'tujuan', data);
                    validation_select('#driver', 'driver', data);
                    ...
                    if (data.status) {
                        ...
                        if (data.koordriver.telp_wa !== '0') {
                            window.open('https://api.whatsapp.com/send?phone=62'+ data.koordriver.telp_wa +'&text=Ada permohonan peminjaman kendaraan baru', 'newwindow'); 
                        }
                    } else {
    ...
    function detail_data(id) {
        $.ajax({
            ...
            success: function(data) {
                var peminjam = data.peminjam;
                $('#voucher, #tujuan, #keterangan, #tgl-start, #tgl-end, #users').attr('disabled', true);
                ...
                $('#tgl-start').val(datetimefmysql(peminjam.tgl_start_peminjaman));
                $('#tgl-end').val(datetimefmysql(peminjam.tgl_end_peminjaman));
                $('#users').val(peminjam.id_atasan).change();
    ...
    function input_driver_km_awal(id) {
        $('#id-booking-km-awal').val(id);
        $('#modal-km-awal').modal('show');
        $('#modal-km-awal-label').text('Form Input KM Awal Kendaraan');
    }

    function input_driver_km_akhir(id) {
        $('#id-booking-km-akhir').val(id);
        $('#modal-km-akhir').modal('show');
        $('#modal-km-akhir-label').text('Form Input KM Akhir Kendaraan');
    }

    function konfirmasi_simpan_km_awal() {
        if ($('#km-awal').val() == '') {
            validation_live('#km-awal', 'Kolom KM Awal Kendaraan harus diisi.')
            return false;
        }

        $.ajax({
            type: 'POST',
            url: baseUrl + '/update_km_awal_kendaraan',
            cache: false,
            data: $('#form-km-awal').serialize(),
            dataType: 'JSON',
            success: function(data) {
                if (data.status) {
                    table_data();
                    $('#modal-km-awal').modal('hide');
                    swalAlert('success', 'Simpan Data', data.message);
                } else {
                    swalAlert('error', 'Simpan Data', data.message);
                }
    ...
    function konfirmasi_simpan_km_akhir() {
        if ($('#km-akhir').val() == '') {
            validation_live('#km-akhir', 'Kolom KM Akhir Kendaraan harus diisi.')
            return false;
        }

        $.ajax({
            type: 'POST',
            url: baseUrl + '/update_km_akhir_kendaraan',
            cache: false,
            data: $('#form-km-akhir').serialize(),
            dataType: 'JSON',
            success: function(data) {
                if (data.status) {
                    table_data();
                    $('#modal-km-akhir').modal('hide');
                    swalAlert('success', 'Simpan Data', data.message);
                    
                    if (data.datadriver) {
                        var pesan = `*Driver :* ${data.datadriver.driver.nama_driver} | *No. Telp :* ${data.datadriver.driver.telp_driver} | *Kendaraan :* ${data.datadriver.driver.nopol} Telah kembali ke pool`;
                        window.open('https://api.whatsapp.com/send?phone=62'+ data.datadriver.traveladmin.telp_wa +'&text=' + pesan + '', 'newwindow'); 
                    }
                } else {
                    swalAlert('error', 'Simpan Data', data.message);
                }
    ...
    function ubah_status(id, status) {
        Swal.fire({
            ...
            text: 'Apakah anda ingin menutup peminjaman kendaraan ini ?',
            icon: 'question',
            showCancelButton: true,
            cancelButtonColor: '#d33',
            confirmButtonColor: '#3085d6',
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.status) {
                            ...
                            // if (data.datadriver.telp_driver !== '0') {
                            //     var pesan =  `<?= base_url('booking/kendaraan') ?>`;
                            //     window.open('https://api.whatsapp.com/send?phone=62'+ data.datadriver.telp_driver +'&text=' + pesan + '', 'newwindow'); 
                            // }
                        } else {
