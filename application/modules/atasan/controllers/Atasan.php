...
class Atasan extends SM_Controller
{
    function __construct()
    {
        ...
        $this->load->model('M_atasan', 'm_atasan');
    }

    function index()
    {
        $this->template->add_title_segment('Masterdata Atasan');

        $data = array(
            'active'     => 'master-atasan',
            'breadcrumb' => 'Masterdata Atasan',
            'modal'      => array('atasan/modal'),
        );
    ...
    function ajax_list()
    {
        ...
        $list  = $this->m_atasan->get_datatables($sort, $order);
        foreach ($list as $l) :
            $no++;
            $l->no   = $no;
            $l->nama = $l->nama;
        ...
        $output = array(
            "draw"              => $_POST['draw'],
            "recordsTotal"      => $this->m_atasan->count_all($sort, $order),
            "recordsFiltered"   => $this->m_atasan->count_filtered($sort, $order),
            "data"              => $data,
        );
    ...
    function ajax_data_id()
    {
        ...
        $data = $this->m_atasan->get_data_by_id($id);
    ...
    function ajax_post() 
    {
        ...
        $data = $this->m_atasan->simpan_data($request);
        if ($data) :
    ...
    function ajax_put()
    {
        ...
        $data = $this->m_atasan->update_data($request);
        if ($data) :
            echo json_encode(array(
                'status' => $data,
    ...
    {
        $id = $this->input->get('id', true);
        $data = $this->m_atasan->hapus_data($id);
        if ($data) :
            echo json_encode(array(
                'status' => $data
            ));
