<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn-tambah').on('click', function() {
            ...
            $('.username-hide').show();
            $('.password-hide').show();
            $('.konfirmasi-password-hide').show();
            $('#modal').modal('show');
            $('#modal-label').html('Form Tambah Users');
        });
        ...
        // validation remove select2
        $('.select2-input').change(function() {
            if ($(this).val() !== '') {
                validation_remove_select2(this);
            }
        });
        ...
        $('#divisi').select2({
            ajax: {
                url: "<?= base_url('auto/get_auto_divisi') ?>",
                dataType: 'json',
                quietMillis: 100,
            ...
            formatResult: function(data) {
                var markup = '<b>' + data.nama + '</b><br/><i>' + data.kategori_divisi + '</i>';
                return markup;
            },
            formatSelection: function(data) {
                return data.nama + ', ' + data.kategori_divisi;
            }
        });

        // cek similarity username

        $('#username').keyup(function() {
            similarity_check($(this).val());
        });
    });
    ...
    function reset_data() {
        $('#id, .reset-form').val('');
        $('#s2id_divisi a .select2-chosen').html('');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.validasi === false) {
                    ...
                    // validation('#username', 'username', data);
                    validation('#password', 'password', data);
                    validation('#re-password', 're_password', data);
                    validation('#email', 'email', data);
                    validation('#telp-wa', 'telp_wa', data);
                    validation_select2('#divisi', 'divisi', data);
                } else {
                    if (data.status) {
                        table_data();
                        $('#modal').modal('hide');
    ...
    function ubah_data(id) {
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.username-hide').hide();
                $('.password-hide').hide();
                $('.konfirmasi-password-hide').hide();

                $('#divisi').val(data.id_divisi);
                $('#s2id_divisi a .select2-chosen').html(data.divisi);
                
                $('#telp-wa').val(data.telp_wa);
                $('#telp').val(data.telp);
                ...
                $('#modal').modal('show');
                $('#modal-label').html('Form Ubah Users');
            },
            error: function(e) {
    ...
    function similarity_check(username) {
        $.ajax({
            type: 'POST',
            url: baseUrl + '/username_similarity_check',
            data: 'username=' + username,
            cache: false,
            dataType: 'JSON',
            success: function(data) {
                if (data.similar) {
                    validation_live('#username', 'Username sudah dipakai, silahkan cari username lain');
                    $('#btn-simpan').attr('disabled', 'disabled');
                } else {
                    validation_remove('#username');
                    $('#btn-simpan').removeAttr('disabled');
                }
            },
            error: function(e) {
                swalAlert('error', e.status, 'Terjadi Kesalahan');
            }
        });
    }
</script>