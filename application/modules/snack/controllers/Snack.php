...
class Snack extends SM_Controller
{
    function __construct()
    {
        ...
        $this->load->model('M_snack', 'm_snack');
    }

    function index()
    {
        $this->template->add_title_segment('Masterdata Snack');

        $data = array(
            'active'     => 'master-snack',
            'breadcrumb' => 'Masterdata Snack',
            'modal'      => array('snack/modal'),
        );
    ...
    function ajax_list()
    {
        ...
        $list  = $this->m_snack->get_datatables($sort, $order);
        foreach ($list as $l) :
            $no++;
            $l->no   = $no;
            $l->nama = $l->nama;
        ...
        $output = array(
            "draw"              => $_POST['draw'],
            "recordsTotal"      => $this->m_snack->count_all($sort, $order),
            "recordsFiltered"   => $this->m_snack->count_filtered($sort, $order),
            "data"              => $data,
        );
    ...
    function ajax_data_id()
    {
        ...
        $data = $this->m_snack->get_data_by_id($id);
    ...
    function ajax_post() 
    {
        ...
        $data = $this->m_snack->simpan_data($request);
        if ($data) :
    ...
    function ajax_put()
    {
        ...
        $data = $this->m_snack->update_data($request);
        if ($data) :
            echo json_encode(array(
                'status' => $data,
    ...
    {
        $id = $this->input->get('id', true);
        $data = $this->m_snack->hapus_data($id);
        if ($data) :
            echo json_encode(array(
                'status' => $data
            ));
